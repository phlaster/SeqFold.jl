const DNA_COMPLEMENT = Comp(('A' => 'T', 'T' => 'A', 'G' => 'C', 'C' => 'G', 'N' => 'N'))
...
"""
const DNA_MULTIBRANCH = MultiBranch((2.6, 0.2, 0.2, 2.0))

"""
The Thermodynamics of DNA Structural Motifs
- SantaLucia and Hicks, 2004
"""
const DNA_NN = BpEnergy((
    "init" => (0.2, -5.7),
    "init_G/C" => (0.0, 0.0),
    "init_A/T" => (2.2, 6.9),
    "sym" => (0, -1.4),
...
"""
const DNA_INTERNAL_MM = BpEnergy((
    "AG/TT" => (1.0, 0.9),
    "AT/TG" => (-2.5, -8.3),
    "CG/GT" => (-4.1, -11.7),
    "CT/GG" => (-2.8, -8.0),
...
"""
const DNA_TERMINAL_MM = BpEnergy((
    "AA/TA" => (-3.1, -7.8),
    "TA/AA" => (-2.5, -6.3),
    "CA/GA" => (-4.3, -10.7),
    "GA/CA" => (-8.0, -22.5),
...
for k in collect(keys(DNA_NN))
    rev_k = reverse(k)
    if !haskey(DNA_NN, rev_k)
        DNA_NN[rev_k] = DNA_NN[k]
    end
end

for k in collect(keys(DNA_INTERNAL_MM))
    rev_k = reverse(k)
    if !haskey(DNA_INTERNAL_MM, rev_k)
        DNA_INTERNAL_MM[rev_k] = DNA_INTERNAL_MM[k]
    end
end

for k in collect(keys(DNA_TERMINAL_MM))
    rev_k = reverse(k)
    if !haskey(DNA_TERMINAL_MM, rev_k)
        DNA_TERMINAL_MM[rev_k] = DNA_TERMINAL_MM[k]
    end
end

for k in collect(keys(DNA_DE))
    rev_k = reverse(k)
    if !haskey(DNA_DE, rev_k)
        DNA_DE[rev_k] = DNA_DE[k]
    end
end
...
"""
const DNA_TRI_TETRA_LOOPS = BpEnergy((
    "AGAAT" => (-1.5, 0.0),
    "AGCAT" => (-1.5, 0.0),
    "AGGAT" => (-1.5, 0.0),
    "AGTAT" => (-1.5, 0.0),
...
"""
const DNA_INTERNAL_LOOPS = LoopEnergy((
    (0, 0),
    (0, 0),
    (0, -10.3),
    (0, -11.6),
...
"""
const DNA_BULGE_LOOPS = LoopEnergy((
    (0, -12.9),
    (0, -9.4),
    (0, -10.0),
    (0, -10.3),
...
"""
const DNA_HAIRPIN_LOOPS = LoopEnergy((
    (0, 0.0),
    (0, 0.0),
    (0, -11.3),
    (0, -11.3),
